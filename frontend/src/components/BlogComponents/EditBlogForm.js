...
const EditBlogForm = () => {
    ...
    useEffect(() => {
        ...
        if (title && title.length > 500 ) validationErrors.title = "Blog title must be shorter than 500 characters"
        if (!body || !body.length) validationErrors.body = "Blog body is required"
        if (body && body.length > 10000 ) validationErrors.body = "Blog body must be shorter than 10000 characters"
